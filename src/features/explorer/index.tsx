...
function formatTransactionType(type: TatCoinTransaction["type"]): string {
  switch (type) {
    case "send":
      return "Send";
    case "delegate":
...
export default function Explorer() {
  ...
  const [transaction, setTransaction] = useState<TatCoinTransaction | null>(
    null,
  );
  const [block, setBlock] = useState<TatCoinBlock | null>(null);
  const [account, setAccount] = useState<TatCoinAddress | null>(null);
  ...
  async function openTransaction(hash: string) {
    try {
      ...
      setSearchError(
        err instanceof Error ? err.message : "Unable to load transaction",
      );
    } finally {
      setSearching(false);
    }
...
useEffect(() => {
  ...
  if (tx) {
    ...
    setSearchValue(hash);
    void openTransaction(hash);
    return;
  }

  if (addressParam) {
    const normalizedAddress = addressParam.trim();

    if (!/^tat1[a-z0-9]+$/.test(normalizedAddress)) {
      setSearchError("Invalid TatCoin address");
      return;
    }

    setSearchValue(normalizedAddress);
    void openAddress(normalizedAddress);
  }
}, [searchParams]);
  ...
  async function openBlock(height: string) {
    try {
      ...
      setSearchError(
        err instanceof Error ? err.message : "Unable to load block",
      );
    } finally {
      setSearching(false);
    }
  ...
  async function openAddress(addressValue: string) {
    try {
      ...
      setSearchError(
        err instanceof Error ? err.message : "Unable to load address",
      );
    } finally {
      setSearching(false);
    }
  ...
  async function handleSearch() {
    ...
    try {
      ...
      setSearchError(err instanceof Error ? err.message : "Unable to search");
    } finally {
      setSearching(false);
    }
  }
            ...
            {transaction.type === "send" ? (
              ...
              <>
                <div>
                  ...
                  {transaction.delegatorAddress ? (
                    <button
                      type="button"
                      onClick={() => {
                        void openAddress(transaction.delegatorAddress!);
                      }}
                      className="mt-2 w-full break-all rounded-2xl border border-white/10 bg-black/10 p-4 text-left font-mono text-sm text-cyan-300 transition hover:border-cyan-400/30 hover:text-cyan-200"
                    >
                      {transaction.delegatorAddress}
                ...
                <div>
                  ...
                  <div className="mt-2 break-all rounded-2xl border border-white/10 bg-black/10 p-4 font-mono text-sm text-violet-300">
      ...
      {account && (
        <div className="max-w-4xl rounded-3xl border border-white/10 bg-white/[0.03] p-6">
          <div>
            ...
            <div className="mt-2 text-lg font-semibold text-white">Details</div>
          </div>
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-white/10 bg-black/10 p-4">
              ...
              <div className="mt-2 font-medium text-white">TatCoin Mainnet</div>

              <div className="mt-1 font-mono text-xs text-cyan-300">tat-1</div>
            </div>
          </div>
        </div>
      )}
          ...
          {accountTransactions.length === 0 ? (
            ...
            <div className="mt-6 space-y-3">
              {accountTransactions.map((tx) => {
                ...
                const delegated = tx.direction === "delegate";
                const undelegated = tx.direction === "undelegate";
                const claimed = tx.direction === "claim_rewards";
                ...
                const amountClass = sent
                  ...
                  : received || claimed
                    ...
                    : delegated
                      ? "text-right font-semibold text-cyan-300"
                      : "text-right font-semibold text-violet-300";
              ...
              <div className="mt-3 flex items-center gap-2">
                {status.catchingUp ? (
                  <>
                    ...
                    <span className="font-medium text-amber-200">Syncing</span>
                  </>
                ) : (
                  <>
                    <RiCheckboxCircleLine className="text-xl text-emerald-300" />
                    <span className="font-medium text-emerald-200">Synced</span>
                  </>
                )}
              </div>
