...
function addTatValues(first: string, second: string): string {
  const total =
    BigInt(tatToUtat(first)) +
    BigInt(tatToUtat(second));
...
export default function Send() {
  ...
  const [estimatingFee, setEstimatingFee] =
    useState(false);
  ...
  async function handleEstimateFee() {
    ...
    try {
      ...
      setEstimatedFee(
        formatTatBalance(result.feeUtat),
      );
    } catch {
      setEstimatedFee(null);
    } finally {
  ...
  async function handleMax() {
    ...
    if (!recipient.startsWith("tat1")) {
      setError(
        "Enter a valid recipient address before using MAX",
      );
      return;
    }
    ...
    try {
      ...
      const balanceUtat =
        BigInt(balanceQuery.data);

      const feeUtat =
        BigInt(estimate.feeUtat);
      ...
      setAmount(
        utatToTat(maxUtat.toString()),
      );

      setEstimatedFee(
        utatToTat(feeUtat.toString()),
      );
    } catch (err) {
      setError(
  ...
  async function handleReview() {
    ...
    try {
      ...
      if (!recipient.startsWith("tat1")) {
        throw new Error(
          "Invalid TatCoin recipient address",
        );
      }
      ...
      <div className="space-y-6">
        ...
        <div className="max-w-2xl rounded-3xl border border-amber-400/20 bg-amber-400/[0.05] p-6">
          <div className="text-sm text-amber-200">
            Create or import a wallet before
            sending TAT.
          </div>
        </div>
      </div>
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Check the transaction details before
            signing.
          </p>
        </div>
